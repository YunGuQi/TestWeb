...
import { Download, ChevronRight, X, ChevronLeft, RefreshCcw, Clock, ArrowRight } from "lucide-react";
import {
  Radar,
  RadarChart,
  PolarGrid,
...
export default function ResultPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const filtered = historyArr.filter((item: any) => item.id !== id);
      filtered.unshift({
        id,
  ...
  return (
    <div className="h-screen bg-zinc-950 text-white font-sans overflow-hidden flex flex-col items-center relative">
      ...
      {/* Bottom Action Area */}
      <div className="fixed bottom-0 left-0 w-full p-4 pb-6 bg-gradient-to-t from-zinc-950 via-zinc-950 to-transparent z-30 pointer-events-none flex flex-col items-center justify-end">
        <div className="w-full max-w-sm flex justify-center gap-3 pointer-events-auto mb-3">
          <button 
        ...
        <div className="flex items-center justify-center gap-4 w-full font-mono font-bold pointer-events-auto">
          <button onClick={() => setShowHistoryModal(true)} className="text-xs text-gray-400 hover:text-white transition-colors tracking-widest underline underline-offset-4">
            查看历史档案
          </button>
          <button onClick={() => router.push('/hub')} className="text-xs text-gray-400 hover:text-white transition-colors tracking-widest underline underline-offset-4">
            探索其他专柜
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showHistoryModal && (
          <motion.div
            ...
            className="fixed inset-0 z-[150] flex items-end justify-center bg-black/80 backdrop-blur-sm p-4 font-sans text-white"
          >
            <motion.div
            ...
            >
              ...
              <div className="space-y-3">
                ...
                {history.length === 0 && (
                  <p className="text-zinc-500 text-sm text-center py-4">暂无历史档案</p>
                )}
              </div>
            </motion.div>
          </motion.div>
